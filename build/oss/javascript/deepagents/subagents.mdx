...
<ParamField body="skills" type="list[str]">
    Skill source paths for the subagent's SkillsMiddleware.
...
### Using the GENERAL_PURPOSE_SUBAGENT constant

The SDK exports a `GENERAL_PURPOSE_SUBAGENT` constant that you can use to create custom variants of the general-purpose subagent:



```typescript
import { createDeepAgent, GENERAL_PURPOSE_SUBAGENT, SubAgent } from "deepagents";

// Create a custom variant with different tools
const customGP: SubAgent = {
  ...GENERAL_PURPOSE_SUBAGENT,
  name: "research-gp",
  tools: [webSearchTool, readFileTool],
  skills: ["/skills/research/"],  // Add custom skills
};
