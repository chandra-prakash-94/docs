...
:::warning
**Avoid Using Tools with Structured Output**

When using `with_structured_output(method="function_calling")`, avoid passing  tools (like `Google Search` [{"google_search": {}}]) directly in the `invoke()` call. 

If the model decides to use a tool (such as searching the web) before generating the structured response, the internal parser will fail to find the expected schema in the initial response and return `None`. This often results in an `AttributeError: 'NoneType' object has no attribute`.
